...
notes, links, files, and images, organized into collections, with a command palette and a
Stripe-gated Pro tier.
...
populated and how the value is rendered. Items are created and viewed in a drag-resizable
side drawer.

**Collections.** Group items of any type. The relation is many-to-many, so one item can live
in several collections, tracked through an explicit join table.
...
"full access until launch"); the gating logic ships and is unit-tested regardless.
...
- **[`.claude/`](.claude/)**: the project's Claude Code setup, with custom subagents
