 ...
 * (kept-dashboard.html's `alpha = dark ? '2E' : '1F'`). Defaults to the dark-mode suffix for
 * any caller that doesn't know the current theme — components rendering theme-dependent
 * chips (ItemCard, CollectionCard, SidebarNav, ItemDrawer) should pass the suffix from
 * `useSoftTintAlpha()` (`src/hooks/`) instead of relying on this default.
 *
